...
const manualsSourceFolder = 'manuals'
...
const dbFile = {}
try {
  if (!fs.existsSync(path.join(rootFolder, manualsSourceFolder))) {
    throw new Error(`Source manuals directory not found at: ${path.join(rootFolder, manualsSourceFolder)}`)
  }
  ...
  for (const lang in sources) {
    ...
    const docsDestFolder = path.join(rootFolder, 'docs')
    if (!fs.existsSync(docsDestFolder)) {
      fs.mkdirSync(docsDestFolder)
    }

    for (const page in sources[lang].pages) {
      const md = new Remarkable()
      const sourceFilePath = path.join(rootFolder, manualsSourceFolder, lang, sources[lang].pages[page].file)

      if (!fs.existsSync(sourceFilePath)) {
        console.warn(`Manual file not found, skipping: ${sourceFilePath}`)
        continue
      }
      
      let input = fs.readFileSync(sourceFilePath, 'utf8')
      ...
      const langDocsFolder = path.join(docsDestFolder, lang)
      if (!fs.existsSync(langDocsFolder)) {
        fs.mkdirSync(langDocsFolder)
      }

      const destFilePath = path.join(langDocsFolder, sources[lang].pages[page].file)
      fs.writeFileSync(destFilePath, `<!--- DO NOT EDIT. This file is automatically generated from ${manualsSourceFolder}/${lang}/${sources[lang].pages[page].file} changes made to this file will be lost -->\n` + mdFile, { flag: 'w+' })
  ...
  if (batch.length > 0) {
    const dbPath = path.join(rootFolder, 'packs', 'system-doc')
    if (fs.existsSync(dbPath)) {
      await ClassicLevel.destroy(dbPath)
    }
    ...
    console.warn('No manuals generated: no valid source files found')
  }
} catch (e) {
  console.error('ERROR generating manuals:', e.message)
}
