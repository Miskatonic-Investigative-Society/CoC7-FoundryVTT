...
* On the range chat-card, the fast option is not default anymore.
  * When targeting a target with MOV of 8+, the fast option will not be selected by default. Instead, selecting fast for a target with MOV less thant 8 will trigger a warning. You will still be able to select it.
