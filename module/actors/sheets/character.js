...
export class CoC7CharacterSheet extends CoC7ActorSheet {
  ...
  async getData() {
    ...
    sheetData.minPulpTalents = this.actor.archetype?.system.talents
      ...
      : 0;
    sheetData.invalidPulpTalents =
      sheetData.pulpTalentCount < sheetData.minPulpTalents;

    sheetData.hasDevelopmentPhase = this.actor.hasDevelopmentPhase;

    sheetData.allowDevelopment = game.settings.get(
      "CoC7",
      "developmentEnabled"
    );
    sheetData.allowCharCreation = game.settings.get(
      "CoC7",
      "charCreationEnabled"
    );
    ...
    sheetData.showDevPannel =
      sheetData.allowDevelopment || sheetData.allowCharCreation;

    sheetData._monetaryFormats = [];
    for (const key in COC7.monetaryFormats) {
      sheetData._monetaryFormats.push({
        key,
        val: game.i18n.localize(COC7.monetaryFormats[key]),
      });
    }

    sheetData.showCurrencySymbol = [
      "decimalLeft",
      "decimalRight",
      "integerLeft",
      "integerRight",
    ].includes(sheetData.data.system.monetary.format);

    sheetData._monetaryTypes = [];
    for (const key in COC7.monetaryTypes) {
      if (
        COC7.monetaryTypes[key].filter.length === 0 ||
        COC7.monetaryTypes[key].filter.includes(
          sheetData.data.system.monetary.format
        )
      ) {
        sheetData._monetaryTypes.push({
          key,
          val: game.i18n.localize(COC7.monetaryTypes[key].name),
        });
      }
    }
    ...
    if (!sheetData.manualCredit) {
      sheetData.monetary = {
        spendingLevel: CoCActor.monetaryFormat(
          sheetData.data.system.monetary.format,
          sheetData.data.system.monetary.symbol,
          this.actor.spendingLevel
        ),
        assets: CoCActor.monetaryFormat(
          sheetData.data.system.monetary.format,
          sheetData.data.system.monetary.symbol,
          this.actor.assets
        ),
        cash: CoCActor.monetaryFormat(
          sheetData.data.system.monetary.format,
          sheetData.data.system.monetary.symbol,
          this.actor.cash
        ),
    ...
    sheetData.oneBlockBackStory = game.settings.get(
      "CoC7",
      "oneBlockBackstory"
    );
    ...
    if (
      sheetData.displayPlayerName &&
      !sheetData.data.system.infos.playername
    ) {
  ...
  _saveScrollPositions(html) {
    ...
    const selectors = [".right .tab.development ol"];
  ...
  _restoreScrollPositions(html) {
    ...
    for (const sel of selectors) {
      const el = html.find(sel);
      el.each((i, el) => {
        el.scrollLeft = positions[sel]?.[i] || 0;
      });
    }
  }
  ...
  _onAddMonetary() {
    const values = this.actor.system.monetary.values
      ? duplicate(this.actor.system.monetary.values)
      : [];
    values.push({
      name: "",
      min: null,
  ...
  _updateObject(event, formData) {
    ...
    if (system.monetary?.values) {
      formData["system.monetary.values"] = Object.values(
        system.monetary.values || []
      );
    }
