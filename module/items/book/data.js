...
export class CoC7Book extends CoC7Item {
  ...
  static convertDifficulty(difficulty) {
    switch(difficulty) {
      case 'hard': return CoC7Check.difficultyLevel.hard
      case 'extreme': return CoC7Check.difficultyLevel.extreme
      case 'critical': return CoC7Check.difficultyLevel.critical
      case 'unreadable': return CoC7Check.difficultyLevel.impossible
      default: return CoC7Check.difficultyLevel.regular
    }
  }
  async _changeProgress(mode, value) {
    if (!this.isOwned) {
      return ui.notifications.error('This item must be owned by an Actor for the Full Study progress count to be done.')
    }
    if (!this.data.data.initialReading) {
      return ui.notifications.error(`${this.actor.name} need to perform an Initial Reading on <i>${this.name}</i> to be able to advance through Full Study progress.`)
    }
  ...
  async _attemptInitialReading () {
    const difficulty = CoC7Book.convertDifficulty(this.data.data.difficultyLevel)
    ...
    if (skill.length === 0) {
      return ui.notifications.error(`${this.actor.name} does not know the ${language} language.`)
    } else {
