...
export class OpposedCheckCard extends RollCard{
	...
	get attackerRoll(){
		if( this.combat){
			if( this.rolls?.length){
				const attacker = this.rolls.filter( r => 0 == r.index|| r.attacker);
	...
	get defenderRoll(){
		if( this.combat){
			if( this.rolls?.length){
				const defender = this.rolls.filter( r => 0 != r.index || !r.attacker);
	...
	get winnerRollsDamage(){
		if( this.combat && this.rolls.length >= 2 && this.hasWinner && 'weapon' == this.winner?.item?.type) return true;
		return false;
	}
	...
	get advantageDefender(){
		if( undefined == this._ad) return this.defenderIsDodging;
		return this._ad;
	}
