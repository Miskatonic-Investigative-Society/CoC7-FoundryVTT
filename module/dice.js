    ...
    if (!isBlind && !hideDice) {
      if (game.modules.get('dice-so-nice')?.active) {
        ...
        tenDie.results.forEach(dieResult => {
          diceResults.push(dieResult === 100 ? 0 : dieResult / 10)
        })
        ...
        tenDie.results.forEach(dieResult => {
          diceData.throws[0].dice.push({
            result: dieResult === 100 ? 0 : dieResult / 10,
            resultLabel: dieResult === 100 ? 0 : dieResult / 10,
            type: 'd100',
            vectors: [],
            options: {
