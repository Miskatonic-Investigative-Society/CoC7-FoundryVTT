...
export class CoC7ChatMessage {
  ...
  static cardTypes (config) {
    ...
    if (config.rollType === CoC7ChatMessage.ROLL_TYPE_ATTRIBUTE && config.attribute === 'san') {
  ...
  static normalizeRequest (options) {
    if (typeof options.event === 'undefined' || typeof options.cardType === 'undefined' || typeof options.rollType === 'undefined') {
      ui.notifications.error(game.i18n.format('CoC7.ErrorNotFound', { value: game.i18n.localize('CoC7.Roll') }))
      return false
    }
    if (![CoC7ChatMessage.CARD_TYPE_COMBINED, CoC7ChatMessage.CARD_TYPE_GROUP, CoC7ChatMessage.CARD_TYPE_NORMAL, CoC7ChatMessage.CARD_TYPE_OPPOSED, CoC7ChatMessage.CARD_TYPE_SAN_CHECK].includes(options.cardType)) {
      ui.notifications.error(game.i18n.format('CoC7.ErrorInvalidFormula', { value: game.i18n.localize('CoC7.ErrorInvalidCardType') }))
      return false
    }
    if (![CoC7ChatMessage.ROLL_TYPE_SKILL, CoC7ChatMessage.ROLL_TYPE_CHARACTERISTIC, CoC7ChatMessage.ROLL_TYPE_ATTRIBUTE].includes(options.rollType)) {
    ...
    switch (config.dialogOptions.rollType) {
      ...
      case CoC7ChatMessage.ROLL_TYPE_ATTRIBUTE:
        config.options.skillId = options.event.currentTarget.closest('.item')?.dataset.skillId
        config.options.itemId = options.event.currentTarget.closest('.item')?.dataset.itemId
        config.options.characteristic = options.event.currentTarget.parentElement.dataset.characteristic
        config.options.attribute = options.event.currentTarget.parentElement.dataset.attrib
        config.dialogOptions.attribute = config.options.attribute
        config.options.actorId = options.event.currentTarget.closest('form').dataset.actorId
        config.options.tokenKey = options.event.currentTarget.closest('form').dataset.tokenId
        config.options.weaponAltSkill = options.event.currentTarget.classList.contains('alternativ-skill')
        config.options.actor = options.actor
        if (config.dialogOptions.rollType === CoC7ChatMessage.ROLL_TYPE_SKILL && config.options.isCombat) {
        ...
        if (typeof config.options.actorId === 'undefined' || typeof config.options.tokenKey === 'undefined' || typeof config.options.actor === 'undefined') {
          ui.notifications.error(game.i18n.format('CoC7.ErrorInvalidFormula', { value: game.i18n.localize('CoC7.ErrorActor') }))
          return false
        }
        if (config.dialogOptions.rollType === CoC7ChatMessage.ROLL_TYPE_SKILL && typeof config.options.skillId === 'undefined') {
          ui.notifications.error(game.i18n.format('CoC7.ErrorInvalidFormula', { value: game.i18n.localize('CoC7.Entities.Skill') }))
          return false
          ...
          if (typeof config.options.characteristic === 'undefined') {
            ui.notifications.error(game.i18n.format('CoC7.ErrorInvalidFormula', { value: game.i18n.localize('CoC7.Characteristic') }))
            return false
          } else if (!game.user.isGM && !config.options.actor.data.data.characteristics[config.options.characteristic]?.value) {
            return false
          }
        } else if (config.dialogOptions.rollType === CoC7ChatMessage.ROLL_TYPE_ATTRIBUTE) {
          if (typeof config.options.attribute === 'undefined') {
            ui.notifications.error(game.i18n.format('CoC7.ErrorInvalidFormula', { value: game.i18n.localize('CoC7.Attribute') }))
            return false
          } else if (!game.user.isGM && !config.options.actor.data.data.attribs[config.options.attribute]?.value) {
            return false
          }
        } else if (config.dialogOptions.rollType === CoC7ChatMessage.ROLL_TYPE_COMBAT) {
          if (typeof config.options.itemId === 'undefined') {
            ui.notifications.error(game.i18n.format('CoC7.ErrorInvalidFormula', { value: game.i18n.localize('CoC7.Entities.Weapon') }))
            return false
          } else if (config.options.altKey) {
            return false
          }
        }
        config.options.hasPlayerOwner = (config.options.actor.hasPlayerOwner ?? false)
        if (config.dialogOptions.rollType === CoC7ChatMessage.ROLL_TYPE_CHARACTERISTIC) {
          config.dialogOptions.disableFlatThresholdModifier = config.options.isCtrlKey
          config.dialogOptions.disableFlatDiceModifier = config.options.isCtrlKey
        } else {
        ...
        config.dialogOptions.difficulty = CoC7Check.difficultyLevel[game.settings.get('CoC7', 'defaultCheckDifficulty')]
  ...
  static async trigger (options = {}) {
    ...
    if (config.options.isCtrlKey && game.user.isGM && [CoC7ChatMessage.CARD_TYPE_NORMAL, CoC7ChatMessage.CARD_TYPE_SAN_CHECK].includes(config.dialogOptions.cardType)) {
  ...
  static async createRoll (config) {
    ...
    if (usage) {
      ...
      config.dialogOptions.flatDiceModifier = Number(usage.get('flatDiceModifier'))
      config.dialogOptions.flatThresholdModifier = Number(usage.get('flatThresholdModifier'))
    }
  }
      ...
      case CoC7ChatMessage.CARD_TYPE_NORMAL:
        {
          ...
          check.actor = !config.options.tokenKey ? config.options.actorId : config.options.tokenKey
      ...
      case CoC7ChatMessage.CARD_TYPE_COMBINED:
        {
          ...
          let data = {}
          if (config.dialogOptions.cardType === CoC7ChatMessage.CARD_TYPE_OPPOSED) {
            data = {
