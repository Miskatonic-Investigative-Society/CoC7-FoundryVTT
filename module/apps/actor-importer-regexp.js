...
const nameCharacters = '\\u3000\\u3400-\\u4DBF\\u4E00-\\u9FFF\\w\\(\\)\\-\\/&"\'' + CoC7Utilities.quoteRegExp('áéíóàèìòùÀÈÌÒÙáéíóúýÁÉÍÓÚÝâêîôûÂÊÎÔÛãñõÃĀÑÕäëïöüÿÄËÏÖÜŸçÇßØøÅåÆæœ“”')
...
const translations = {
  en: {
    ...
    db: '(?<![a-z])(?:' + keys.en.fulldb + '):?\\s+(?<db>[+-]?\\d+(?:d\\d+|D\\d+)?|' + keys.en.dbNone + ')[,\\s\n]*',
    build: '(?<![a-z])' + 'Build' + ':?\\s+(?<build>[+-]?\\d+)[,\\s\n]*',
    armor: '(?<![a-z])' + 'Armor' + ':?\\s+(?<armor>' + keys.en.armorNone + '|\\d+)[,\\s\n]*',
  ...
  fr: {
    ...
    san: '(?<![a-z])(?:' + 'SAN|Santé Mentale' + '):?\\s+(?<san>\\d+|-)[,\\s\n]*',
    hp: '(?<![a-z])(?:' + 'PV|Points de vie' + '):?\\s+(?<hp>\\d+|-)[,\\s\n]*',
    mp: '(?<![a-z])(?:' + 'PM|Points de magie' + '):?\\s+(?<mp>\\d+|-)[,\\s\n]*',
    db: '(?<![a-z])(?:' + keys.fr.fulldb + '):?\\s+(?<db>[+-]?\\d+(?:d\\d+|D\\d+)?|' + keys.fr.dbNone + ')[,\\s\n]*',
    build: '(?<![a-z])' + 'Carrure' + ':?\\s+(?<build>[+-]?\\d+)[,\\s\n]*',
    armor: '(?<![a-z])' + 'Armure' + ':?\\s+(?<armor>' + keys.fr.armorNone + '|\\d+)[,\\s\n]*',
    mov: '(?<![a-z])(?:' + 'Mvt|Mouvement|Déplacement' + '):?\\s*(?<mov>\\d+)[,\\s\n]*',
  ...
  es: {
    ...
    hp: '(?<![a-z])(?:' + 'PV|Puntos de vida|P\\. ?V\\.' + '):?\\s+(?<hp>\\d+|-)[,\\s\n]*',
    mp: '(?<![a-z])(?:' + 'PM|Puntos Mágicos|Puntos de Magia' + '):?\\s+(?<mp>\\d+|-)[,\\s\n]*',
    db: '(?<![a-z])(?:' + keys.es.fulldb + '):?\\s+(?<db>[+-]?\\d+(?:d\\d+|D\\d+)?|' + keys.es.dbNone + ')[,\\s\n]*',
    build: '(?<![a-z])' + 'Corpulencia' + ':?\\s+(?<build>[+-]?\\d+)[,\\s\n]*',
    armor: '(?<![a-z])' + 'Armadura' + ':?\\s+(?<armor>' + keys.es.armorNone + '|\\d+)[,\\s\n]*',
  ...
  'zh-TW': {
    ...
    hp: '(?<![a-z])(?:' + 'HP|Hit points|生命' + '):?\\s*(?<hp>\\d+|-)[,\\s\n]*',
    mp: '(?<![a-z])(?:' + 'MP|Magic points' + '):?\\s*(?<mp>\\d+|-)[,\\s\n]*',
    db: '(?<![a-z])(?:' + keys['zh-TW'].fulldb + '):?\\s*(?<db>[+-]?\\d+(?:d\\d+|D\\d+)?|' + keys['zh-TW'].dbNone + ')[,\\s\n]*',
    build: '(?<![a-z])' + 'Build' + ':?\\s*(?<build>[+-]?\\d+)[,\\s\n]*',
    armor: '(?<![a-z])' + 'Armor' + ':?\\s*(?<armor>' + keys['zh-TW'].armorNone + '|\\d+)[,\\s\n]*',
    mov: '(?<![a-z])' + 'Move' + ':?\\s*(?<mov>\\d+)[,\\s\n]*',
    lck: '(?<![a-z])' + 'Luck' + ':?\\s*(?<lck>\\d+|-)[,\\s\n]*',
